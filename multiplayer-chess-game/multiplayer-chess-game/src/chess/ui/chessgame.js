...
class ChessGame extends React.Component {
    ...
    componentDidMount() {
        ...
        socket.on('opponent move', move => {
            // move == [pieceId, Cur_pos]
            console.log("opponenet's move: " + move.FigureKEY + ", " + move.Cur_pos)
           if (move.playerColorThatJustMovedIsWhite !== this.props.color) {
               console.log(move.FigureKEY, move.Cur_pos, this.state.gameState)
               this.movePiece(move.FigureKEY, move.Cur_pos, this.state.gameState, false)
               this.setState({
        ...
        if (isMyMove) {
            ...
            console.log(Number(localStorage.getItem('userid')),)
               ...
               nextPlayerColorToMove: !this.state.gameState.thisPlayersColorIsWhite,
                ...
                PlayerID: Number(localStorage.getItem('userid')),
                FigureKEY: selectedId,
                Cur_pos: finalPosition,
              ...
              {items.map(item => (
                  <div key={item.pk}>
                      {item.Status === "wait" && users.isAuthenticated ? (
                          <div>{item.Name} {item.WhitePlayerID}
                              <textarea
